...
class Agent {
    ...
    updateTool(oldToolName, newTool) {
        const toolIndex = this.tools.findIndex(tool => tool.name === oldToolName);
        if (toolIndex !== -1) {
            this.tools[toolIndex] = newTool;
        }
    }

    updateSkill(oldSkillName, newSkill) {
        const skillIndex = this.skills.findIndex(skill => skill.name === oldSkillName);
        if (skillIndex !== -1) {
...
// Save agentic frameworks to local storage
function saveAgenticFramework(agent) {
    const frameworks = JSON.parse(localStorage.getItem("agenticFrameworks")) || [];
    frameworks.push(agent);
    localStorage.setItem("agenticFrameworks", JSON.stringify(frameworks));
}

// Load agentic frameworks from local storage
function loadAgenticFrameworks() {
    return JSON.parse(localStorage.getItem("agenticFrameworks")) || [];
}
...
function removeToolFromAgent(agent, toolName) {
    agent.removeTool(toolName);
}

function removeSkillFromAgent(agent, skillName) {
    agent.removeSkill(skillName);
}

function updateToolInAgent(agent, oldToolName, newTool) {
    agent.updateTool(oldToolName, newTool);
}

function updateSkillInAgent(agent, oldSkillName, newSkill) {
    agent.updateSkill(oldSkillName, newSkill);
}
